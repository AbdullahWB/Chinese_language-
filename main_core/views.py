...
def settings(request):
    ...
    return render(request, 'main_core/settings.html')
...
def inbox(request):
    ...
    connection_users.sort(key=lambda x: (
        ...
        x['last_message_time'] or timezone.make_aware(datetime.min)  # Then by last message time
    ), reverse=True)
...
@login_required
def check_online_status(request, user_id):
    try:
        user = CustomUser.objects.get(id=user_id)
        is_online = user.last_login and (timezone.now() - user.last_login).seconds < 300
